...
harvestingCWD = function(pop, centroids, h.permits, h.time, h.radius, h.num, thyme){
  ...
  if(thyme %in% h.time){
    h.loc = sample(dim(centroids)[1], h.permits, replace = TRUE)
      ...
      S.rem.fid = sample(S.in.sub, S.rem, replace = FALSE)
      E.rem.fid = sample(E.in.sub, E.rem, replace = FALSE)
      I.rem.fid = sample(I.in.sub, I.rem, replace = FALSE)
      ...
      for(f in 1:length(fams.rem.index)){
        ...
        harvest.out.temp = c(time = thyme, loc = pop.out[fams.rem.index[f], 3], S = S.rem, E = E.rem, I = I.rem)
