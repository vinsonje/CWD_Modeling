...
struct MoveLoop : public Worker {
  ...
  MoveLoop(const NumericMatrix& apop,
           ...
           const IntegerMatrix& maxden,
           NumericMatrix outpop) 
    : apop(apop), apopabund(apopabund), apoplocs(apoplocs), acent(acent), maxden(maxden), outpop(outpop) {}
  ...
  arma::imat convertmaxden()
  {
    RMatrix<int> tmp_mat = maxden;
    const arma::imat maxden2(tmp_mat.begin(), tmp_mat.nrow(), tmp_mat.ncol(), false);
    return maxden2;
  }
  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    arma::imat maxden = convertmaxden();
    ...
    for(std::size_t j = begin; j < end; j++) {
      ...
      if(pop_j_3 > 0 & pop_j_0 > 0){
        ...
        if(setsize>0){
          ...
          ////////////////////////////////////////////////////////
          //individuals move to cell with least abundance
          ///////////////////////////////////////////////////////
          ...
          arma::uvec cellindarma = set.elem(find(abundmask==1)); //This line is what sets the cellindarma to the subset of cells with the minimum.
          
          
          ////////////////////////////////////////////////////////
          //individuals move to cell that has a density less than the max
          ///////////////////////////////////////////////////////
          
          // arma::ivec abundmask(abund.n_rows);
          // 
          // //Return indices of set which are less than or equal to the maxden
          // for(std::size_t p = 0; p < abund.n_rows; ++p){
          //   if(abund(p)<<maxden) abundmask[p]=1; //set poplocs mask to 1 if any loc matches cell in set
          //   else abundmask[p]=0;
          // }
          // 
          // for(std::size_t p = 0; p < abund.n_rows; ++p){
          ...
          // }
          // 
          // //then, cellindarma should be wherever mask==1 (ie, wherever the value was equal to the minimum)
          // imat abundinset_s = apopabund3.rows(find(setmask==1));
          // 
          // arma::uvec cellindarma = set.elem(find(abundmask==1)); //This line is what sets the cellindarma to the subset of cells with the minimum.
          
          ////////////////////////////////////////////////////////
          //individuals move to a random cell in range
          ///////////////////////////////////////////////////////
          // arma::uvec cellindarma = set; 
...
NumericMatrix parallelMovementRcpp_portion(const NumericMatrix& apop,
  ...
  MoveLoop moveloop(apop,apopabund,apoplocs,acent,maxden,outpop);
...
NumericMatrix MovementRcppParallel(NumericMatrix& apop,
                                   ...
                                   NumericMatrix& acent,
                                   IntegerVector& maxden) {
  
  Rcpp::NumericMatrix popout = apop;
  //set present locations to previous locations
  popout(_,6)=popout(_,2);
  
  popout(_,2)=parallelMovementRcpp_portion(apop,apopabund,apoplocs,acent,maxden);
